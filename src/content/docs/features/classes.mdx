...
<Aside type="caution">
Chamar um método sem `pub` de fora da classe resulta em erro de compilação:

```soyuz
val r = Retangulo { largura: 4.0, altura: 3.0 }
r.calcularDiagonal()  // ERRO — método 'calcularDiagonal' de 'Retangulo' é privado
```

Adicione `pub` ao método para permitir acesso externo.
</Aside>
